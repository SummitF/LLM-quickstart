...
#     summary = ai_message_content[summary_start:summary_end].strip()
    ...
    # 使用正则表达式提取 content 和 summary 的内容（冒号后可能有空格）
    content_match = re.search(r'content\s*:\s*"(.*?)"', ai_message_content)
    summary_match = re.search(r'summary\s*:\s*"(.*?)"', ai_message_content, re.DOTALL)
